...
void        Component::writeHeader(DataFile &file, recsize_t size, offset_t redirect) const {
    file.write(&index);
    file.write(&size);
    file.write(&redirect);
}

DataRecordHeader*     Component::readHeader(DataFile &file) {
    DataRecordHeader *tempHeader = new DataRecordHeader;
